...
case 'listar':
		$rspta=$usuarios->listar();
 		...
 		while ($reg=$rspta->fetch_object()){
 			$data[]=array(
 				"6"=>($reg->estado)?'<button class="btn btn-warning" onclick="mostrar('.$reg->idusuario.')"><i class="fa fa-pencil"></i></button>'.
 				...
 				"5"=>($reg->estado)?'<span class="label bg-primary">Activado</span>':'<span class="label bg-danger">Desactivado</span>'
 				);
 		}
 		$results = array(
