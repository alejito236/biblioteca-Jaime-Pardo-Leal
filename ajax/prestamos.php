...
switch ($_GET["op"]){
	case 'guardaryeditar':
		if (empty($idprestamo)){
			$rspta=$prestamo->insertar($idcliente,$usuario,$fprestamo,$monto,$formapago,$fechapago,$plazo,$fplazo);
			echo $rspta ? "Prestamo registrado" : "Prestamo no se pudo registrar";
		}
		else {
			$rspta=$prestamo->editar($idprestamo,$idcliente,$usuario,$fprestamo,$monto,$formapago,$fechapago,$plazo,$fplazo);
...
*/
	...
	$rspta=$prestamo->listar();
 		...
 		while ($reg=$rspta->fetch_object()){
 			$data[]=array(
               "5"=>'<button class="btn btn-warning" onclick="mostrar('.$reg->idprestamo.')"> <i class="fa fa-pencil"> </i></button>'.
                ...
                "3"=>$reg->fechaf,
 				"4"=>($reg->estado)?'<span class="label bg-success">Activado</span>':
