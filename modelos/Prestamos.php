...
Class Prestamo
{
    ...
    public function insertar($idcliente,$usuario,$fprestamo,$monto,$fechapago,$fplazo)
    {
        $sql="INSERT INTO prestamos (idcliente,usuario,fprestamo,monto,fpago,fplazo,estado) 
        VALUES ('$idcliente','$usuario','$fprestamo','$monto','$fechapago','$fplazo','1')";
        return ejecutarConsulta($sql);
    }
    
    //Implementamos el metodo para Editar Registros
    public function editar($idprestamo,$idcliente,$usuario,$fprestamo,$monto,$fechapago,$fplazo)
	{
	...
	{
		$sql="SELECT p.idprestamo,c.nombre as cliente,u.nombre as usuario,DATE(p.fprestamo) as fecha,p.monto,DATE(p.fpago) as fechap,p.fplazo as fechaf,p.estado 
